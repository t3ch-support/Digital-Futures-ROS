  ...
  public:
    ...
    void convertImage(const sensor_msgs::ImageConstPtr& msg)
    {
      ...
      catch (cv_bridge::Exception& e)
      {
        ...
        calibrateCamera(cv_ptr);
    }

    void calibrateCamera(cv_bridge::CvImagePtr incomingImage){
        vector< int > ids;
        ...
        if(refindStrategy) cv::aruco::refineDetectedMarkers(incomingImage->image, board, corners, ids, rejected);

        // interpolate charuco corners
        Mat currentCharucoCorners, currentCharucoIds;
        if(ids.size() > 0)
            aruco::interpolateCornersCharuco(corners, ids, incomingImage->image, charucoboard, currentCharucoCorners,
                                             currentCharucoIds);

        // draw results
        if(ids.size() > 0){
            cv::aruco::drawDetectedMarkers(incomingImage->image, corners);
        }

        if(currentCharucoCorners.total() > 0) {
            cv::aruco::drawDetectedCornersCharuco(incomingImage->image, currentCharucoCorners, currentCharucoIds);
        }

        cv::putText(incomingImage->image, "Press 'c' to add current frame. 'ESC' to finish and calibrate",
                Point(10, 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 0, 0), 2);

        cv::imshow(OPENCV_WINDOW, incomingImage->image);
        char key = (char)cv::waitKey(10);
        if(key == 'c' && ids.size() > 0) {
            ROS_INFO_STREAM("Frame Captured");
            allCorners.push_back(corners);
            allIds.push_back(ids);
            allImgs.push_back(incomingImage->image);
            imgSize = incomingImage->image.size();
        }
